...
+ A Rmd file captures a snapshot of the data with a fixed set of parameters
  ...
  - Bad for looking at real-time data (e.g. "Sydney airport" is hard-coded)


+ A Shiny app hides all the computations in the background and only shows inputs and outputs more relevant to the user (a domain expert not familiar with the underlying R codes)

+ A Shiny app can access data in real-time and modify outputs instantly based on user inputs

+ The purpose of this lecture is to build this Shiny app:
...
+ Shiny uses R for computation but impose strict rules on input and output (so much so it almost feels like Shiny is a language on its own)

---
class: segue

Knitting `bomrang.Rmd` will give you a skeleton Shiny app

(This is not a full app, but only to see what an "input" and an "output" are in context of a Shiny app) 
---
# Structure of a Shiny app: `input` and `output`
...
+ You are allowed to have more than two scripts, if you wish to better organise longer codes (e.g. using `global.R`, see later)
---
class: segue
...
+ I recommend building a `Rmd` file before making a full app:
  - Ensure all R codes are correct (debugging Shiny is consierably more difficult)
  - Get a good design of what are the allowable inputs and outputs
...
Demo2: `reactive`, a key concept in Shiny apps
...
- Dont's
  + Write awfully long functions, try to modularise your code whereever possible
...
+ [RStudio course on Shiny](https://github.com/rstudio-conf-2020/shiny-start-finish)
+ [Dr Wishart's course on Shiny](https://github.com/jrwishart/SSA-Shiny-Workshop-2019)
